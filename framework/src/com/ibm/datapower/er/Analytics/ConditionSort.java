...
public class ConditionSort implements Comparator<ConditionsNode> {
	...
	public int compare(ConditionsNode o1, ConditionsNode o2) {
		...
		String condO1 = "";
		String condO2 = ""; 
		if ( o1.getSortConditionName().equals("condensecondition") && o2.getSortConditionName().equals("condensecondition"))
		{
		...
		if (o1.getSortType() == ConditionSortType.TIMESTAMP && 
				o1.getSortOption().length() > 0 ) {
		...
		try {
			if (condO1 != null && condO2 != null) {
				if (!o1.getSortMethod().equals("descending")) {
					...
					if (useTimestampCompare)
						return fmt.parse(condO2).compareTo(fmt.parse(condO1));
					else
					{
						try
						{
